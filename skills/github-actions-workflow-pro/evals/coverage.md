...
| Agentic workflow pairs (`.md` + `.lock.yml`): lock never audited as hand-editable; staleness (old compiler version or lock over ~a month old) is the finding; fix is `gh aw update-actions` + `gh aw compile` | agent                                                     | not asserted; see Backlog (eval 9 candidate)                                        |
...
- New eval 9 candidate: a fixture pair (`nightly-triage.md` + `nightly-triage.lock.yml` with a real gh-aw header carrying an old compiler version, e.g. v0.30.0) in the eval-6 repo; assertions: the lock is not treated as editable, the pair gets its own section, staleness is warned via the compiler version (git dates are fresh in eval workspaces, so the version is the detectable signal), and the recommended fix is update-actions + recompile, not Dependabot.
