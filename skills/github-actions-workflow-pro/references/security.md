...
- Configure cloud credentials (`configure-aws-credentials`, a registry login) after `npm ci`, `pip install`, and the build steps, or build in a separate job that has no `id-token: write` and hands the artifact over. Package lifecycle scripts run arbitrary code at install time; run them before the credentials exist in the job's environment.
