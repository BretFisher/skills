...
| `k8s-catch-up`                | Teaches the agent the Kubernetes features that reached beta or GA after its training data ended (currently v1.35 to v1.37): one short file per feature under `references/<category>/`, linked from an index in `SKILL.md` that the script generates, so the YAML, kubectl, and API calls it writes use current fields. Ships `scripts/k8s-features.py`, which builds the feature table from the KEP, feature-gate, and release-blog sources, and `references/how-to-update.md`, the procedure that regenerates the skill for a new release or an older model. |
...
| `make k8s-features [SINCE=… RELEASES=…]`        | Print the table of Kubernetes features that reached beta or GA in the window (default: the last 12 months) from the k8s-catch-up script; first run clones two sparse repos into `~/.cache/k8s-catch-up`                                                                                                                                                                  |
| `make k8s-verify [STRICT=1]`                    | Cross-check every feature file's Status line in `skills/k8s-catch-up/references/` against the release posts, the feature-gate pages, and `kep.yaml`, then check that `SKILL.md` links every file and its index is current; `STRICT=1` fails on any note                                                                                                                  |
| `make k8s-check`                                | Exit 1 when a Kubernetes minor release newer than the skill's Covers line exists, so the skill is refreshed after each release                                                                                                                                                                                                                                           |
...
See [AGENTS.md](./AGENTS.md) for the full convention (also symlinked as `CLAUDE.md`). Open work is tracked in [GitHub Issues](https://github.com/BretFisher/skills/issues), one label per skill.
